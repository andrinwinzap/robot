...
CallbackReturn RobotSystem::on_init(const hardware_interface::HardwareInfo &info)
{
  ...
  try 
  {
    ...
    position_publishers_.resize(num_joints);
    ...
    for (size_t i = 0; i < num_joints; i++)
    {
      ...
      // Publisher to micro-ROS set_position topic
      std::string set_topic = "/robot/" + joint_name + "/set_position";
      position_publishers_[i] = node_->create_publisher<std_msgs::msg::Float32>(set_topic, 10);
      ...
      RCLCPP_INFO(node_->get_logger(), 
                  ...
                  joint_name.c_str(), set_topic.c_str(), get_position_topic.c_str(), get_velocity_topic.c_str());
    }
  }
...
CallbackReturn RobotSystem::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // Reset joint states and commands
  {
    ...
    for (auto &pos : joint_positions_) pos = 0.0;
    for (auto &vel : joint_velocities_) vel = 0.0;
    for (auto &cmd : joint_commands_) cmd = 0.0;
  }
...
{
  // Send ros2_control joint commands to micro-ROS topics
  for (size_t i = 0; i < info_.joints.size(); i++)
  {
    const std::string cmd_name = info_.joints[i].name + "/" + hardware_interface::HW_IF_POSITION;
    joint_commands_[i] = get_command(cmd_name);

    // Publish command to micro-ROS
    std_msgs::msg::Float32 msg;
    msg.data = static_cast<float>(joint_commands_[i]);
    position_publishers_[i]->publish(msg);
    
    // For debugging - can be removed in production
    RCLCPP_DEBUG(node_->get_logger(), 
                "Commanding joint %s to position: %f", 
                info_.joints[i].name.c_str(), joint_commands_[i]);
  }
